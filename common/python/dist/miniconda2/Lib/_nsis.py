...
def read_url(fn):
    try:
        data = open(join(sys.prefix, 'pkgs', 'urls.txt')).read()
        urls = data.split()
        for url in urls[::-1]:
            if url.endswith(fn):
                return url
...
def create_conda_meta():
    ...
    meta['url'] = read_url('%(name)s-%(version)s-%(build)s.tar.bz2' % meta)
...
def mk_dirs():
    if not os.path.exists(join(sys.prefix, 'envs')):
...
def add_anaconda_to_path():
    ...
    add_to_system_path([sys.prefix,
                        join(sys.prefix, 'Scripts'),
                        join(sys.prefix, "Library\\bin")], allusers)
    broadcast_environment_settings_change()
