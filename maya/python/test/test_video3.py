...
class VideoPlayerProgressBar(ProgressBar):
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(pos=self._update_bubble, size=self._update_bubble, seek=self._update_bubble)
...
class VideoPlayer(GridLayout):
    ...
    def on_play(self, instance, value):
        if self._video is None:
            self._video = Video(source=self.source, play=True, volume=self.volume, pos_hint={'x': 0, 'y': 0}, **self.options)
            self._video.bind(texture=self._play_started, duration=self.setter('duration'), position=self.setter('position'), volume=self.setter('volume'))
